...
export function extractFilePathsFromCodebaseSearchOutput(output: string): string[] {
	const results: string[] = []
	const re = /^File path:\s*(.+)\s*$/gm
...
export async function recordCodebaseSearchSnapshots(input: {
	cwd: string
	taskId: string
	toolName: ToolName
	toolResult: string
}): Promise<void> {
	if (input.toolName !== "codebase_search") return

	const intentId = await resolveActiveIntentId(input.cwd)
	if (!intentId) return

	const filePaths = extractFilePathsFromCodebaseSearchOutput(input.toolResult)
	...
	for (const relPath of filePaths.slice(0, MAX_FILES)) {
		try {
			...
			readHashStore.set(
				...
				{ hash: sha256Hash(content), capturedAt: new Date().toISOString(), toolName: "codebase_search" },
			)
		} catch {
			// best-effort
		}
